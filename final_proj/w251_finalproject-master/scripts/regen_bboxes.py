...
for i,datafile_line in enumerate(csv.reader(fh)):
    ...
    # id,path,x,y,box_w,box_h,w,h,label,dir,english,bird_group,isdir
    ...
    x_center = round((int(image_data[2]) + int(image_data[4])/2) / int(image_data[6]),6)
    y_center = round((int(image_data[3]) + int(image_data[5])/2) / int(image_data[7]),6)
    
    # now the height and width
    height = round(int(image_data[5]) / int(image_data[7]),6)
    width = round(int(image_data[4]) / int(image_data[6]),6)
